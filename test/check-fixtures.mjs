...
function *walkNode(node) {
  ...
  for (const key of ["lhs", "rhs", "cond", "then", "else", "init", "inc",
                     "casAddr", "casOld", "casNew", "atomicExpr"]) {
...
function *walkProgramNodes(data) {
  for (const obj of data.ast.globals) {
    if (obj.isFunction && obj.body)
      yield *walkNode(obj.body);
  }
}
...
const requireAssertCount = (data, relpath) => {
  ...
  if (actual !== expected)
    fail(`${relpath}: ASSERT count mismatch: expected ${expected}, got ${actual}`);
};

const requireFunctions = (data, relpath, ...names) => {
  const funcs = new Set(
    data.ast.globals
      .filter((obj) => obj.isFunction)
      .map((obj) => obj.name),
  );
...
const requireVlaSupport = (data, relpath) => {
  ...
  const hasVlaNode = [...walkProgramNodes(data)].some((node) => node.kind === "ND_VLA_PTR");
...
const requireToken = (data, relpath, lexeme, kind) => {
  const found = data.tokens.some((tok) => tok.lexeme === lexeme && tok.kind === kind);
  if (!found)
...
const requireTokenKind = (data, relpath, kind) => {
  ...
  if (!found)
    fail(`${relpath}: missing token kind ${kind}`);
};
...
const astCases = {
  "test/arith.c": (data) => {
    ...
    requireNodeKinds(
      ...
      "ND_SHL", "ND_SHR", "ND_COND", "ND_COMMA", "ND_CAST",
    );
  },
  "test/control.c": (data) => {
    ...
    requireNodeKinds(
      data, "test/control.c",
      "ND_IF", "ND_FOR", "ND_DO", "ND_SWITCH",
      "ND_CASE", "ND_GOTO", "ND_LABEL",
      "ND_GOTO_EXPR", "ND_LABEL_VAL",
    );
  },
